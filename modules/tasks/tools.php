			...
			):?>
			{
				...
				onclick : function(event, item)
				{
					...
					if (BX.hasClass(item.layout.item, "task-menu-popup-item-add-deadline"))
					{
						...
						if(typeof window.COMPANY_WORKTIME != 'undefined')
							deadline = BX.CJSTask.addTimeToDate(deadline, window.COMPANY_WORKTIME);
		...
		if ( ! $bSkipJsMenu )
		{
			...
			if ($arAllowedTaskActions['ACTION_ADD_FAVORITE'])
			{
				?>{
					...
					title : "<?php echo GetMessage("ACTION_ADD_FAVORITE")?>", 
					className : "task-menu-popup-item task-menu-popup-item-favorite", 
					onclick : function() {
						window.top.AddToFavorite(<?=intval($task["ID"])?>);
						this.popupWindow.close();
					}
				},
				<?php
			...
			if ($arAllowedTaskActions['ACTION_DELETE_FAVORITE'])
			{
				?>{
					...
					title : "<?php echo GetMessage("ACTION_DELETE_FAVORITE")?>", 
					className : "task-menu-popup-item task-menu-popup-item-favorite", 
					onclick : function() {
						window.top.DeleteFavorite(<?=intval($task["ID"])?>, {mode: 'delete-subtree', rowDelete: <?=($inFavorite ? 'true' : 'false')?>});
						this.popupWindow.close();
					}
				},
				<?php

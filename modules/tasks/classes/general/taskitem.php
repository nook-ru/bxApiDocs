...
use Bitrix\Tasks\Task\FavoriteTable;
...
final class CTaskItem implements CTaskItemInterface
{
	...
	private function proceedAction($actionId, $arActionArguments = null)
	{
		...
		if ($actionId == self::ACTION_ADD_FAVORITE || $actionId == self::ACTION_DELETE_FAVORITE || $actionId == self::ACTION_TOGGLE_FAVORITE)
		{
			...
			// in case of a ticked came, get all child tasks IDs here, pass ID list to \Bitrix\Tasks\Integration\Socialnetwork\Task::toggleFavorites()
	...
	public static function getManifest()
	{
		...
		if ($arReadableTaskDataKeys === null)
		{
			...
			$arSortableTaskDataKeys = array_unique(array_merge($arCTasksManifest['REST: readable task data fields'], $arCTasksManifest['REST: writable task data fields'], $arCTasksManifest['REST: filterable task data fields']));

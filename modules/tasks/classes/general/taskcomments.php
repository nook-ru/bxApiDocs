...
class CTaskComments
{
	...
	public static function onAfterCommentAdd($entityType, $entityId, $arData)
	{
		...
		// please someone refactor this ugly function!
			...
			foreach ($arRecipientsIDs as $userID)
			{
				...
				if ($isExtranetEnabled)
				{
					...
					else
					{
						if($arTask["GROUP_ID"])
						{
							$groupSite = CSocNetGroup::GetSite($arTask['GROUP_ID'])->fetch(); // intranet site for that group
							$userSiteId = $groupSite['LID'];
							$arSites[$userSiteId] = $groupSite;
						}
						else
						{
				...
				$arMessageFields = array(
					...
					"NOTIFY_TAG" => "TASKS|COMMENT|".intval($arTask['ID'])."|".intval($userID),
	...
	public static function onAfterCommentUpdate($entityType, $taskID, $arData)
	{
		...
		if($arData['ACTION'] == 'EDIT' || $arData['ACTION'] == 'DEL')
		{
			...
			else
			{
				...
				$messageAuthorId = (is_object($GLOBALS['USER']) && method_exists($GLOBALS['USER'], 'getId') ? $GLOBALS['USER']->getId() : CTasksTools::GetCommanderInChief());//$arData['PARAMS']['AUTHOR_ID'];
			...
			$arLogFields = array(
				"TASK_ID" => (int) $taskID,
				"USER_ID" => static::getOccurAsUserId($messageAuthorId),

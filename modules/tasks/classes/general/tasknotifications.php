...
class CTaskNotifications
{
	...
	)
	{
		...
		// find two sites: INTRANET and EXTRANET
		$arSites = array();
		...
		if (is_array($arRecipientsIDs) && count($arRecipientsIDs))
		{
			...
			while ($arUser = $rsUser->Fetch())
			{
				...
				$arMessageFields = array(
					...
					"NOTIFY_TAG" => "TASKS|TASK|".intval($taskID)."|".intval($arUser['ID'])
				);
		...
		elseif (CModule::IncludeModule("crm"))
		{
			$dbCrmActivity = CCrmActivity::GetList(
				...
				array(
					...
					'ASSOCIATED_ENTITY_ID' => $arTask["ID"],
					'CHECK_PERMISSIONS' => 'N'
				),
				false,
				false,
	...
	public static function GetNotificationPath($arUser, $taskID, $bUseServerName = true, $arSites = array())
	{
		...
		{
			// try to make a kind of "site cache" from useless $arSites
			$siteCache = array();
			if(is_array($arSites) && !empty($arSites))
			{
				if(is_array($arSites['INTRANET']))
				{
					$siteCache[$arSites['INTRANET']['SITE_ID']] = $arSites['INTRANET'];
				}

				if(is_array($arSites['EXTRANET']))
				{
					$siteCache[$arSites['EXTRANET']['SITE_ID']] = $arSites['EXTRANET'];
				}
			}

			// we have extranet and the current user is an extranet user
			$bExtranet = 	CModule::IncludeModule('extranet') && !CTaskNotifications::__isIntranetUser($arUser["ID"]);
			// task is in a group
			$useGroup = 	$arTask['GROUP_ID'] && CTasksTools::HasUserReadAccessToGroup($arUser['ID'], $arTask['GROUP_ID']);
			...
			else
			{
				if($useGroup)
				{
					$groupSite = CSocNetGroup::GetSite($arTask['GROUP_ID'])->fetch(); // intranet site for that group in case of multi-division
					if($groupSite)
					{
			...
			if(!is_array($siteCache[$siteID])) // still no site??? abort!
				return false;
